...
def generate_round_robin_schedule():
    ...
    Matches are distributed across rounds based on the number of available courts,
    ensuring no more than num_courts matches are scheduled simultaneously.
    ...
    for team1_id, team2_id in all_pairings:
        ...
        # Create match record
        match = {
            "id": len(matches) + 1,      # Unique match ID
            "round": round_num,           # Round number
            "team1": team1_id,            # First team ID
            "team2": team2_id,            # Second team ID
            "score1": None,               # Team 1 score (set when match is played)
            "score2": None,               # Team 2 score (set when match is played)
            "winner": None,               # Winner team ID (determined after match)
            "status": "scheduled"         # Match status: scheduled, completed, or bye
        }
